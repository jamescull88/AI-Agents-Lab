...
def get_order_by_id(req: func.HttpRequest) -> func.HttpResponse:
    order = [
        ...
        if orderr["orderId"] == int(req.route_params.get("orderId"))
    ]
    ...
    if len(order) > 0:
        ...
        return func.HttpResponse(
            json.dumps({"status": "order_not_found"}), status_code=404
        )


@app.route(
    route="orders", auth_level=func.AuthLevel.ANONYMOUS, methods=[func.HttpMethod.POST]
)
